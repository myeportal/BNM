...
function NFTBalance() {
  ...
  useEffect(() => {
    if (isInitialized && isAuthenticated) {
      getNFTBalances({
        params: { chain: "0x89", address: user.get("ethAddress") },
        onSuccess: (nftBalance) => {
          Promise.all(
            nftBalance.result.map(async (nft, index) => {
              // this means moralis hasn't synced yet, do it ourselfs
              if (nft.is_valid === 0 || (!!!nft.metadata && !!nft.token_uri)) {
              ...
              return (
                <Card
                  ...
                  actions={[
                    ...
                    <Tooltip title="Transfer NFA" key={index}>
                      <SendOutlined
                        onClick={() => {
                          setNftToSend(nft);
                  ...
                  cover={
                    <Image
                      ...
                      src={
                        (nft?.metadata &&
                          (JSON.parse(nft?.metadata).image?.startsWith(
                            "ipfs://"
                          )
                            ? "https://gateway.pinata.cloud/ipfs/" +
                              JSON.parse(nft?.metadata).image.split(
                                "ipfs://"
                              )[1]
                ...
                >
                  {/* {console.log(nft)} */}
                  <Meta
                    title={`${nft?.metadata && JSON.parse(nft?.metadata).name}`}
